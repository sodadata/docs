...
| `--set soda.library.idle=true` | (Optional) Launch an idle worker so at scan time, the agent can hand over instructions to an already running idle Scan Launcher to avoid the start-from-scratch setup time for a pod. You can have multiple idle scan launchers waiting for instructions. |
  `--set soda.library.replicas=1` | (Optional) Replicate an idle worker to have more workers ready to handle instructions without setting up a new pod. |
