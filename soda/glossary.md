...
### metric
A property of the data in your database. See [Metrics]({% link soda-sql/sql_metrics.md %}).

### monitor
A set of details you define in Soda Cloud which Soda SQL uses when it runs a scan. For a new monitor, you define: a dataset and column against which to execute a test, a test, an alert, a notification, an owner, and a description. See [Scans]({% link soda/scan.md %}).
...
### warehouse YAML
The file in which you configure warehouse connection details and Soda Cloud connection details. See [Warehouse YAML]({% link soda-sql/warehouse.md %}) and [Connect to Soda Cloud]({% link soda-cloud/connect_to_cloud.md %}).