...
This example offers instructions for both a Soda-hosted agent deployment model which uses Soda Cloud connected to a Soda Agent to securely access data sources and execute scheduled scans for data quality anomaly detections. See: [Choose a flavor of Soda]({% link soda/setup-guide.md %}#soda-hosted-agent). 
...
3. Navigate to the **Data source** tab, then click **New Data Source** and follow the guided steps to connect to a new data source. <br />Refer to the subsections below for insight into the values to enter in the fields and editing panels in the guided steps. 
