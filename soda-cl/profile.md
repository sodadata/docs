...
* **Spark**: Soda uses the profiling include/exclude pattern to build the query that retrieves a dataset's metadata, but because Spark does not support this activity. For profiling on data in a Spark data source, Soda performs a `show tables` action to retrieve all the datasets in a schema, then filters the list based on the include/exclude pattern defined by the user. This filtering works with regular expressions, replacing `%` with `.*` and performing a full regex match. If you have issues with discovery or profiling on data in Spark, change your profiling include  or exclude pattern(s) to `tablename\_.*` so that it is the regex equivalent of a SQL pattern that Spark can use.
