...
checks for dim_customer:
  - distribution_difference(number_cars_owned) > 0.05:
      ...
      # example for postgres the following query randomly samples 50% of the data 
      # with seed 61
...
<details>
  ...
  If you wish, you can define your own sample size using a SQL query. See [Define the sample size](https://docs.soda.io/soda-cl/distribution.html#define-the-sample-size), below.
</details>
...
6. (Optional), to sample the data in the distribution check for **continuous columns** only, replace the value of `sample` with a query that specifies the portion of the data in your dataset for which you are checking the distribution. The data source you are using must support the query you write. <br />For example, for PostgreSQL, you can use the `TABLESAMPLE` clause to randomly sample 50% of the data with seed 61. Best practice dictates that you use sampling for large datasets that might not fit in memory. Refer to the [define the sample size](#define-the-sample-size) for details. If `sample` or `filter` is not used in the distribution check for **continuous columns**, Soda fetches up to 1 million records by applying `limit` clause for better memory management. For **categorical columns**, `sample` is not supported.
...
* For categorical columns, the value counts of each category has been calculated in the database and Soda fetches the aggregated category and value counts. If there are more than one million distinct categories, Soda skips the distribution check and issues a warning.
...
Use `sample` for only continuous values! For categorical values refer to [Distribution check details](### Distribution-check-details).
