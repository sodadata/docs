...
To review the failed rows in Soda Cloud, navigate to the **Checks** dashboard, then click the row for a check for duplicate values. Examine failed rows in the **Failed Rows Analysis** tab; see [Manage failed row samples]({% link soda-cl/failed-row-samples.md %}) for further details.
...
| `duplicate_count`  | The count of distinct values that have duplicates. <br> Multiple column names can be specified to count duplicate sets of values, as in `duplicate_count(cola, colb)` <br />See also: [Duplicate check]({% link soda/quick-start-sodacl.md %}#duplicate-check) | number<br /> text<br /> time | all  |
| `duplicate_percent`  | `duplicate_count` (as defined above) over the total row count, expressed as a percentage. <br />See also: [Duplicate check]({% link soda/quick-start-sodacl.md %}#duplicate-check)                                                                             | number<br /> text<br /> time | all  |
