...
[Access to datasets and monitors](#access-to-datasets-and-monitors)<br />
[Default roles for datasets and monitors](#default-roles-for-datasets-and-monitors)<br />
...
By default, when any member adds a new dataset to the Soda Cloud account via Soda SQL or Soda Core , Soda Cloud *does not* assign the **Everyone** group to the dataset. You can adjust this setting to automatically assign the **Everyone** group to each newly added dataset; see [Change the default access to datasets](#change-the-default-access-to-datasets).
...
* By default, when a dataset's Admin or Manager grants another member or the Everyone group access to a dataset, Soda Cloud automatically assigns the new member or group the [default role of Editor for the dataset](#default-roles-for-datasets-and-monitors).  You can adjust this setting to a different default role.
* By default, when any member adds a new dataset to the Soda Cloud account via Soda SQL or Soda Core , Soda Cloud *does not* assign the **Everyone** group to the dataset. You can adjust this setting to automatically assign the **Everyone** group to each newly added dataset.
...
<details>
    ...
    <ol>
      ...
      <li>Then, using Soda SQL or Soda Core , I connect to a new data source, and upload 20 new datasets to Soda Cloud.</li>
    ...
    a) changing it individually on an individual dataset or monitor.<br />
    b) changing its default role in the Organization Settings which <i>only</i> applies when the Everyone group is added to a dataset or monitor on upload or creation.
</details>
 

## Change access to a dataset

When any member uses Soda SQL or Soda Core  to add a new dataset to the Soda Cloud account, the Member automatically becomes the Dataset Owner. The new dataset can only be accessed by an Admin and the Dataset Owner, who automatically becomes a Manager of the dataset, until the Admin or Dataset Owner changes access to the dataset to grant other members access.
...
* **Authors** essentially have read-write access in Soda Cloud and maintain the role of Admin, Manager, or Editor relative to monitors and datasets. See [Roles and rights](#roles-and-rights) above.
* **Viewers** essentially have read-only acecss in Soda Cloud and maintain the role of Viewer relative to monitors and datasets. See [Roles and rights](#roles-and-rights) above.
...
* By default, the member who created the [API keys]({% link soda-sql/connect_to_cloud.md %}) that link Soda SQL or Soda Core  to Soda Cloud becomes the **Dataset Owner** of all existing datasets that already had connections to Soda SQL or Soda Core . The default role that Soda Cloud assigns to the Dataset Owner is that of Manager.
* By default, any member who uses Soda SQL or Soda Core  to add a new dataset to the Soda Cloud account becomes the Dataset Owner. The default role that Soda Cloud assigns to the Dataset Owner is that of Manager.
